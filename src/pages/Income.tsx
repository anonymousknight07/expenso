...
const Income = () => {
  ...
  const calculateSalaryAfterLeaves = () => {
    ...
    const regularIncomes = currentMonthIncomes.filter(
      (income) => !income.is_holiday
    );
    const totalIncome = regularIncomes.reduce(
      (sum, income) => sum + (income.in_hand_amount || income.amount),
      0
    );
  ...
  const exportToExcel = () => {
    ...
    const incomesByMonth = incomes.reduce((acc, income) => {
      ...
      acc[monthYear].push({
        ...
        "In-Hand Amount": income.in_hand_amount
          ? `${currency.symbol}${income.in_hand_amount.toFixed(2)}`
          : "-",
    ...
    Object.entries(incomesByMonth).forEach(([monthYear, monthIncomes]) => {
      ...
      const totalAmount = monthIncomes.reduce(
        (sum, inc) => sum + parseFloat(inc.Amount.replace(currency.symbol, "")),
        0
      );
  ...
  const exportToPDF = () => {
    ...
    Object.entries(incomesByMonth).forEach(([monthYear, monthIncomes]) => {
      ...
      const totalAmount = monthIncomes.reduce(
        (sum, inc) => sum + (inc.in_hand_amount || inc.amount),
        0
      );
      ...
      monthIncomes.forEach((income) => {
        ...
        doc.text(
          `${currency.symbol}${(income.in_hand_amount || income.amount).toFixed(
            2
          )}`,
          150,
          yPos
        ...
        if (income.is_holiday) {
          doc.text(
            `Holiday: ${income.holiday_type} - ${income.holiday_reason}`,
            30,
            yPos
          );
          yPos += 7;
  ...
  const shareFile = (type: "whatsapp" | "email") => {
    if (type === "whatsapp") {
      window.open(
        `https://wa.me/?text=Check out my income report: ${shareUrl}`
      );
    } else {
  ...
  const getDailyPayForDisplay = () => {
    const regularIncomes = incomes.filter(
      (income) => !income.is_holiday && income.in_hand_amount
    );
  ...
  const handleEditIncome = (income: Income) => {
    ...
    setNewIncome({
      ...
      in_hand_amount: income.in_hand_amount?.toString() || "",
    });
  ...
  const handleUpdateIncome = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase
      ...
      .update({
        ...
        date: newIncome.date,
        category_id: newIncome.category_id,
        in_hand_amount: newIncome.in_hand_amount
          ? parseFloat(newIncome.in_hand_amount)
          : null,
      })
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="flex flex-col gap-6 mb-8">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          ...
          <div className="flex flex-wrap gap-4">
            ...
            <Button onClick={() => setIsAddingIncome(true)} variant="primary">
              <Plus className="w-5 h-5 mr-2" />
              Add Income
            </Button>
            <Button onClick={() => setIsAddingHoliday(true)} variant="outline">
              <Plus className="w-5 h-5 mr-2" />
              Add Holiday
            </Button>
          </div>
        ...
        <div className="grid md:grid-cols-3 gap-6">
          ...
          <div className="bg-white rounded-lg shadow p-6">
            ...
            <p className="text-3xl font-bold text-yellow-500">
              {currency.symbol}
              {getDailyPayForDisplay().toFixed(2)}
            </p>
          ...
          <div className="bg-white rounded-lg shadow p-6">
            ...
            <p className="text-3xl font-bold text-blue-500">
              {currency.symbol}
              {calculateSalaryAfterLeaves().toFixed(2)}
            </p>
            <p className="text-gray-600">
              {leaveCount} leave(s) taken ({currency.symbol}
              {(getDailyPayForDisplay() * leaveCount).toFixed(2)} deducted)
            </p>
          </div>
        </div>
      </div>
            ...
            <form onSubmit={handleAddIncome} className="space-y-4">
              <div>
                <label className="block text-sm font-medium mb-1">
                  In-Hand Amount ({currency.symbol})
                </label>
                <input
                  type="number"
                  step="0.01"
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setNewHoliday({
                      ...newHoliday,
                      holiday_type: e.target.value as "leave" | "festival",
                    })
                  }
                  className="w-full px-3 py-2 border rounded"
                  required
      ...
      {isEditingIncome && selectedIncome && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white p-6 rounded-lg w-full max-w-md">
                  ...
                  <div className="flex items-center gap-2">
                    {!income.is_holiday && (
                      <span className="text-lg font-semibold">
                        {currency.symbol}
                        {(income.in_hand_amount || income.amount).toFixed(2)}
                      </span>
                    )}
          ...
          <div className="grid grid-cols-7 gap-1">
            ...
            {Array.from({ length: 35 }, (_, i) => {
              ...
              const dayIncomes = incomes.filter(
                (income) => income.date === dateStr
              );
              const isHoliday = dayIncomes.some((income) => income.is_holiday);
              const totalAmount = dayIncomes.reduce(
                (sum, income) =>
                  sum +
                  (income.is_holiday
                    ? 0
                    : income.in_hand_amount || income.amount),
                0
              );
